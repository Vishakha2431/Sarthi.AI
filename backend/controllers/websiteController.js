...
const getWebsiteById = async (req, res) => {
 ...
 const deployWebsite = async(req, res)=>{
   try {
    ...
    const frontendUrl = rawFrontendUrl && rawFrontendUrl.toLowerCase() !== "undefined"
      ? rawFrontendUrl.replace(/\/+$/, "")
      : "http://localhost:5173" // fallback if FRONTEND_URL is not configured or is literally "undefined"

    website.deployed = true
    website.deployedUrl = `${frontendUrl}/site/${website.slug}`
    await website.save()
...
module.exports={getBySlug,deployWebsite,changeWebsite,getWebsiteById,getAllWebsite,generateWebsite}